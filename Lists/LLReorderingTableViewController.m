//
//  LLReorderingTableViewController.m
//  Lists
//
...
@interface LLReorderingTableViewController ()
...
@implementation LLReorderingTableViewController
...
- (void)viewDidLoad {
	 ...
	 */
	__weak LLReorderingTableViewController *blockSelf = self;
	if ( resignActiveObserver == nil )
		resignActiveObserver = [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationWillResignActiveNotification object:nil queue:nil usingBlock:^(NSNotification *arg1) {
			if ( [blockSelf isDraggingCell] ) {
				LLReorderingTableViewController *strongBlockSelf = blockSelf;
